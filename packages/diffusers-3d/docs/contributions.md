...
public checkpoint APIs, and deterministic resume identity. Training manifest schema version `5` records canonical
objective settings, optimizer/scheduler/batch/precision/seed settings, and the effective LoRA adapter initialization
seed, plus the exact selected and frozen component types and configurations.
`LoRAFineTune.adapter_seed=None` inherits `TrainingConfig3D.seed`; changing any recorded identity value is a resume
